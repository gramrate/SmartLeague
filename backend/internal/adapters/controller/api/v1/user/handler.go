...
func (h *handler) Setup(router *echo.Group) {
	...
	router.GET("/user/all", h.GetAllByFilter)
	router.GET("/user/:id/games", h.GetUserGames, h.authMiddleware.OptionalAuth)
	router.GET("/user/:id/series", h.GetUserSeries, h.authMiddleware.OptionalAuth)
